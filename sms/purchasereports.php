...
<div class="row-fluid">
    ...
    <div class="green box"><label>Received Date</label>
					<input type="text" id="inputField1" name="rcvcdate"  />
	</div>
		<div class="pink box">
...
{
	$query	=	"SELECT p.noofitems,p.billno,p.billdate,p.rcvcno,p.rcdate,s.supplier_name,i.item_name,u.unit_name FROM purchase_tbl p join  supplier_tbl s on p.supplier_id=s.supplier_id join item_tbl i on p.item_id=i.item_id join unit_tbl u on p.unit_id=u.unit_id where p.supplier_id='".$supplier_id."'  ORDER BY i.item_name";
}
...
{
	$query	=	"SELECT p.noofitems,p.billno,p.billdate,p.rcvcno,p.rcdate,s.supplier_name,i.item_name,u.unit_name FROM purchase_tbl p join  supplier_tbl s on p.supplier_id=s.supplier_id join item_tbl i on p.item_id=i.item_id join unit_tbl u on p.unit_id=u.unit_id where p.item_id='".$item_id."'  ORDER BY i.item_name";
}
$res	=	mysql_query($query);
}
...
{
	$query	=	"SELECT p.noofitems,p.billno,p.billdate,p.rcvcno,p.rcdate,s.supplier_name,i.item_name,u.unit_name FROM purchase_tbl p join  supplier_tbl s on p.supplier_id=s.supplier_id join item_tbl i on p.item_id=i.item_id join unit_tbl u on p.unit_id=u.unit_id where p.rcdate=STR_TO_DATE('".$rcdate."', '%d-%M-%Y') ORDER BY i.item_name";
}
$res	=	mysql_query($query);
...
{
	$query	=	"SELECT p.noofitems,p.billno,p.billdate,p.rcvcno,p.rcdate,s.supplier_name,i.item_name,u.unit_name FROM purchase_tbl p join  supplier_tbl s on p.supplier_id=s.supplier_id join item_tbl i on p.item_id=i.item_id join unit_tbl u on p.unit_id=u.unit_id where p.rcvcno='"."RV/".$rcvcno."/NIMS"."' ORDER BY p.rcvcno";
}
$res	=	mysql_query($query);
}
...
{
	$query	=	"SELECT p.noofitems,p.billno,p.billdate,p.rcvcno,p.rcdate,s.supplier_name,i.item_name,u.unit_name FROM purchase_tbl p join  supplier_tbl s on p.supplier_id=s.supplier_id join item_tbl i on p.item_id=i.item_id join unit_tbl u on p.unit_id=u.unit_id ORDER BY i.item_name";
}
$res	=	mysql_query($query);
}
...
<tr>
	<th>S.No</th><th>Supplier Name</th><th>Item Name</th><th>Unit Name</th><th>Quantity</th><th>Bill No</th><th>Bill Date</th><th>RC No</th><th>RC Date</th>
</tr>
<?php  
...
?>
<tr>
	<td><?php echo (++$i) ?></td><td><?php echo $row['supplier_name'];?></td><td><?php echo $row['item_name'];?></td><td><?php echo $row['unit_name'];?></td><td><?php echo $row['noofitems'];?></td><td><?php echo $row['billno'];?></td><td><?php echo $row['billdate'];?></td><td><?php echo $row['rcvcno'];?></td><td><?php echo $row['rcdate'];?></td>
</tr>
